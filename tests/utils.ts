...
export async function createMintIfRequired(
  ...
  const mintAccount = await splProgram.account.mint.fetchNullable(
    mint.publicKey
  );
  if (mintAccount == null) {
    await splProgram.methods
      .initializeMint(6, mintAuthority, null)
      .accounts({
        mint: mint.publicKey,
...
export async function createToken(
  ...
  await splProgram.methods
    ...
    .signers([token])
    .preInstructions([
      await splProgram.account.account.createInstruction(token),
    ])
    .rpc();
}
